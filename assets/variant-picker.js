...
export default class VariantPicker extends Component {
  ...
  updateVariantPicker(newHtml) {
    ...
    // Auto-select first available option for any option group that doesn't have a valid selection
    this.autoSelectAvailableVariants();

    return newProduct;
  }

  /**
   * Auto-selects the first available variant option for each option group.
   * This ensures all options are always set to available variants.
   */
  autoSelectAvailableVariants() {
    // Get all option groups (fieldsets or select elements)
    const fieldsets = this.querySelectorAll("fieldset");
    const selects = this.querySelectorAll("select");

    // Handle button/swatch variants in fieldsets
    fieldsets.forEach((fieldset) => {
      ...
      // If no input is checked or the checked one is not available, select the first available
      if (!checkedInput || !fieldset.contains(checkedInput)) {
        const firstAvailable = Array.from(inputs).find(
    ...
    // Handle dropdown variants in select elements
    selects.forEach((select) => {
      const selectedOption = select.querySelector("option[selected]");
      const hasValidSelection =
        selectedOption &&
        Array.from(select.options).some((opt) => opt === selectedOption);

      // If no valid selection exists, select the first available option
      if (!hasValidSelection && select.options.length > 0) {
